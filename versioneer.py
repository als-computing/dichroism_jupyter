...
def git_pieces_from_vcs(tag_prefix, root, verbose, run_command=run_command):
    ...
    if "-" in git_describe:
        ...
        if (branch_name == "main"):
            # Count distance only along `main`; ignore merge paths
    ...
    ##################################################
    #  Added for `develop` branch from `main`
    # `HEAD` might have branched from untagged commit on `main`
    merge_base_main, rc = run_command(
        GITS, ["merge-base", "main", "HEAD", "-a"], cwd=root)
    distance_to_main, rc = run_command(
        GITS, ["rev-list", "--count", "--ancestry-path",
        "%%s..HEAD" %% merge_base_main], cwd=root)
    pieces["distance-to-main"] = int(distance_to_main)

    ##################################################
    #  Added for branches other than `main`
    if pieces["closest-tag"]:
        # TAG exists
        main_to_tag, rc = run_command(
            GITS, ["rev-list", "--count", "--ancestry-path", "--first-parent",
                   "%%s..%%s" % (pieces["closest-tag"], merge_base_main) ],
            cwd=root)
        pieces["main-to-tag"] = int(main_to_tag)
    else:
        pieces["main-to-tag"] = pieces["distance"]
...
def render_pep440_develop(pieces):
    ...
    Our goal: MERGE-POINT.post.devN[+gHEX.dirty]
        +) MERGE-POINT = Most recent common ancestor for `develop` and `main`
    ...
    if pieces["closest-tag"]:
        rendered = pieces["closest-tag"]
        distance_to_merge = pieces["distance-to-main"]
        # distance_merge_to_tag = (pieces["distance"] - distance_to_merge)
        distance_merge_to_tag = pieces["main-to-tag"]
...
def render_pep440_feature(pieces):
    ...
    Our goal: MERGE-POINT.post.devN+gHEX.BRANCH-NAME.M[.dirty]
        +) MERGE-POINT = Most recent common ancestor for `develop` and `main`
          *) Does not yet handle branch from `release-*`
        +) N = DISTANCE from the MERGE-POINT of `develop` and `main`
